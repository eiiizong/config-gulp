...
(function() {
  var getMobileInfo = function() {
    ...
    if (os == 'iOS') {
      ...
      model = md.mobile();
      var newMobile = MobileDevice.getModels().join(' or ');
      // 通过屏幕尺寸判断
      if (newMobile == 'unknown') {
        if (window.screen) {
          var width = screen.width;
          var height = screen.height;
          var resolution = MobileDevice.getResolution();
          switch (resolution) {
            case '640x1136':
              model = 'iPhone 5/iPhone 5s/iPhone SE';
              break;
            case '750x1334':
              model = 'iPhone 6/Phone 6s/iPhone 7/iPhone 8';
              break;
            case '1242x2208':
              model = 'iPhone 6 Plus/Phone 6s Plus/iPhone 7 Plus/iPhone 8 Plus';
              break;
            case '1125x2436':
              model = 'iPhone X/Phone Xs';
              break;
            case '828x1792':
              model = 'iPhone Xr';
              break;
  ...
  if (window.getMobileInfo == undefined) {
    window.getMobileInfo = getMobileInfo;
  }
})();
